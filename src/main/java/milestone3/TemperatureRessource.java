...
/**
 * Creates a simple Temperature Ressource that will update the Temperature Value.
 * @author Sven Andresen
 *
 */
public class TemperatureRessource extends Ressource<Long> implements ArduinoListener {
    ...
    static{
        ...
        payloadTemplates.put(
	        ...
	        "itm:ourPi pit:hasComponent itm:tempSensor .\n" +
	        "itm:tempSensor rds:type pit:Component .\n" +
	        "itm:tempSensor pit:isType \"TEMP\"^^xsd:string .\n" +
	        "itm:tempSensor pit:isActor \"false\"^^xsd:boolean .\n" +
	        "itm:tempSensor pit:hasDescription \"Sensor that gets the current Temperature value\"^^xsd:string .\n" +
	        "itm:tempSensor pit:hasURL \"coap://141.83.175.252:5683/temp\"^^xsd:anyURI .\n" +
	        "itm:tempSensor pit:hasStatus itm:tempSensorState .\n" +
	        "itm:tempSensorState pit:hasValue \"%.2f\"^^xsd:string .\n" +
	        "itm:tempSensor pit:lastModified \"%s\"^^xsd:dateTime .\n" +
	        "itm:tempSensorState pit:hasScaleUnit \"Celcius\"^^xsd:string .\n"
        );        
    }
    ...
    private double lastReadTempValue = -1.0; //as default indicating none has been read yet
	...
	/**
	 * Create a Temperature Ressource.
	 * @param path
	 * @param updateInterval
	 * @param executor
	 */
	public TemperatureRessource(String path, int updateInterval, ScheduledExecutorService executor) {
        ...
        String title = "\"Temp Value (updated every " + updateInterval + " seconds)\"";
	...
	public byte[] getSerializedResourceStatus(long contentFormat) {
        ...
        if (template == null) {
            ...
            return String.format(template, lastReadTempValue, fmt.format(new Date())).getBytes(CoapMessage.CHARSET);
        }
	}
	...
	protected void processGet(SettableFuture<CoapResponse> responseFuture, CoapRequest coapRequest,
